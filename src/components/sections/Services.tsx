...
import {
  Target, Zap, Share2, Palette, Layers, Code,
  Smartphone, Shield, Cpu, Database, Globe, ArrowUpRight,
} from 'lucide-react'
import type { LucideIcon } from 'lucide-react'

const ICON_MAP: Record<string, LucideIcon> = {
  Target, Zap, Share2, Palette, Layers, Code,
  Smartphone, Shield, Cpu, Database, Globe
}
...
export const Services = ({ dict }: { dict: any }) => {
  ...
  return (
    <section id="solutions" className="relative py-32 px-6 bg-[#030303] overflow-hidden flex flex-col items-center justify-center">
      <div className="max-w-7xl w-full z-10">
        <div className="mb-20">
          <h2 className="text-6xl md:text-8xl font-[1000] text-white uppercase italic tracking-tighter leading-[0.8]">
            {dict.title1} <span className="text-[#27DFE9]">{dict.title2}</span>
          </h2>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-6">
          {dict.items.map(({ id, title, icon, size, desc }: any, i: number) => {
            ...
            return (
              ...
              >
                <div className="absolute inset-[-100%] animate-[spin_5s_linear_infinite] opacity-30 group-hover:opacity-100 transition-opacity duration-500 bg-[conic-gradient(from_0deg,transparent_0deg,transparent_90deg,#27DFE9_180deg,transparent_270deg,transparent_360deg)]" />

                <div className="relative h-full w-full bg-[#0a0a0a] rounded-[23px] p-10 z-10 flex flex-col justify-between overflow-hidden">
                  ...
                  <div className="flex justify-between items-start relative z-20">
                    ...
                    <span className="text-[10px] font-mono text-gray-700 tracking-[0.2em]">[{id}]</span>
                  </div>
                  ...
                  <div className="mt-12 relative z-20">
                    ...
                    <p className="text-[9px] text-gray-500 uppercase tracking-[0.4em] mt-3 font-medium">{desc}</p>
                  </div>
