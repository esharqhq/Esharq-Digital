  ...
  "group/button inline-flex shrink-0 items-center justify-center border border-transparent bg-clip-padding text-sm font-medium whitespace-nowrap transition-all outline-none select-none focus-visible:border-ring focus-visible:ring-3 focus-visible:ring-ring/50 active:not-aria-[haspopup]:translate-y-px disabled:pointer-events-none disabled:opacity-50 aria-invalid:border-destructive aria-invalid:ring-3 aria-invalid:ring-destructive/20 dark:aria-invalid:border-destructive/50 dark:aria-invalid:ring-destructive/40 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
  {
    variants: {
      variant: {
        ...
        brand:
          "relative rounded-none bg-[#27DFE9] text-black font-black uppercase italic tracking-wider overflow-hidden",
        brandOutline:
          "rounded-none border-2 border-[#27DFE9] text-[#27DFE9] font-black uppercase italic tracking-wider hover:bg-[#27DFE9] hover:text-black transition-all duration-500",
      },
      size: {
        default:
        ...
        lg: "h-14 gap-2 px-10 text-lg has-data-[icon=inline-end]:pr-3 has-data-[icon=inline-start]:pl-3",
        xl: "h-18 gap-3 px-12 text-xl shadow-[0_0_30px_rgba(39,223,233,0.3)]",
        icon: "size-10",
        "icon-xs":
...
function Button({
  ...
  return (
    ...
    >
      {isBrand && (
        <div className="absolute inset-0 bg-white translate-y-full group-hover/button:translate-y-0 transition-transform duration-500 pointer-events-none" />
      )}
      <span className={cn("relative z-10 flex items-center gap-2 transition-colors")}>
        {children}
      </span>
    </ButtonPrimitive>
  );
